...
class ChunkRankingDataProcessor:
    ...
    def process_chunks(self):
        """
        Parses the JSON to create a dataset for Chunk Ranking.
        Returns:
        ...
        # Identifying feature keys (excluding strings like 'chunk_id')
        # We look for the first valid entry with chunk scores
        sample_chunk = None
        for entry in self.raw_data:
            if entry.get('chunk_scores'):
                sample_chunk = entry['chunk_scores'][0]
                break
        
        if sample_chunk is None:
            raise ValueError("No chunk scores found in any entry.")

        self.feature_names = [k for k, v in sample_chunk.items() 
        ...
        groups: Dict[int, List[int]] = {} # {question_index: [row_indices]}
        labels: Dict[int, int] = {} # {row_index: 1 (correct) or 0 (incorrect)}

        global_idx = 0

        for q_idx, entry in enumerate(self.raw_data):
            correct_ids = set(entry.get('correct_chunk_ids', []))
            
            # Skip entries with no correct chunks or no scores
            if not correct_ids or not entry.get('chunk_scores'):
                continue
            ...
            for chunk in entry['chunk_scores']:
                ...
                feats = [chunk.get(f, 0.0) for f in self.feature_names]
                all_features.append(feats)
                
                # Determine label
                is_correct = 1 if chunk['chunk_id'] in correct_ids else 0
...
def optimize_weights(json_data: List[Dict]) -> tuple[Optional[Dict[str, float]], Optional[ChunkRankingDataProcessor]]:
    try:
        processor = ChunkRankingDataProcessor(json_data)
        
        # Prepare data
        print("Processing data...")
        X, idx_pos, idx_neg = processor.process_chunks()
    except ValueError as e:
        print(f"Skipping optimization: {e}")
        return None, None

    if len(idx_pos) == 0:
        print("No valid pairs found (e.g., all chunks are correct or all are incorrect).")
        return None, None
...
def compute_feature_contributions(
    ...
    """
    Computes the total contribution of each feature across the top 5 chunks per question.
    For each question, calculates the final score for each chunk using optimized weights,
    selects the top 5 chunks, and then calculates feature contributions only for those top 5.
    
    Args:
        json_data: Complete dataset with chunk scores
    ...
    for entry in json_data:
        if not entry.get('chunk_scores'):
            continue
        
        # Calculate final score for each chunk using optimized weights
        chunk_scores: List[tuple[Dict, float]] = []
        for chunk in entry['chunk_scores']:
            final_score = sum(
                chunk.get(feature_name, 0.0) * optimized_weights.get(feature_name, 0.0)
        ...
        top_5_chunks = chunk_scores[:5]
        
        # Calculate contributions only for top 5 chunks
        for chunk, _ in top_5_chunks:
            total_chunks += 1
            
            for feature_name in feature_names:
                feature_value = chunk.get(feature_name, 0.0)
                weight = optimized_weights.get(feature_name, 0.0)
                contribution = feature_value * weight
                feature_contributions[feature_name] += contribution
    
    print(f"\nProcessed {total_chunks} chunks (top 5 per question) for impact analysis")
...
def visualize_feature_impact(feature_contributions: Dict[str, float], output_path: Optional[str] = None) -> None:
    ...
    plt.title('Feature Impact on Top 5 Chunks per Question\n(Total Contribution: Weight × Score across top 5 chunks per question)', 
...
if __name__ == "__main__":
    ...
    sub_dirs = [
        # "Local_HotpotQA_1k",
        #"HotpotQA_1k",
        #"MultiHopRAG_new_config",
        #"Hotpot_1k_BFS",
        #"Dev_set_rankings"
        #"Scaling-DocAwareHybridRAG"
        #"MultiHopRAG_DocAwareHybridRAG-v1"
        #"NovelQA-Rankings"
    ...
    if all_data:
        ...
        if results and processor:
            print("\n" + "="*30)
            print("COMPUTING FEATURE IMPACT ON TOP 5 CHUNKS PER QUESTION")
            print("="*30)
            ...
            print("\nFeature Contributions (Total: Score × Weight across top 5 chunks per question):")
