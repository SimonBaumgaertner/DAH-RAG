...
# Removed vLLM import - using transformers instead
...
class HippoRAG:
    ...
    def index(self, docs: List[str]):
        ...
        # Track existing entities before insertion to identify newly inserted ones
        existing_entity_ids = set(self.entity_embedding_store.get_all_ids())
        self.entity_embedding_store.insert_strings(entity_nodes)
        # Get newly inserted entities (difference between all entities and existing ones)
        all_entity_ids = set(self.entity_embedding_store.get_all_ids())
        newly_inserted_entity_ids = list(all_entity_ids - existing_entity_ids)
        logger.info(f"Found {len(newly_inserted_entity_ids)} newly inserted entities out of {len(all_entity_ids)} total entities.")
        ...
        if num_new_chunks > 0:
            ...
            self.add_synonymy_edges(newly_inserted_entity_ids=newly_inserted_entity_ids)
    ...
    def add_synonymy_edges(self, newly_inserted_entity_ids: Optional[List[str]] = None):
        """
        ...
        Parameters:
            newly_inserted_entity_ids: Optional[List[str]]
                List of entity IDs that were newly inserted in the current indexing batch.
                If None, falls back to comparing all entities (for backwards compatibility, but causes quadratic scaling).
                If empty list, skips synonymy edge creation.
        ...
        all_entity_node_keys = list(self.entity_id_to_row.keys())

        # Determine which entities to use as queries
        if newly_inserted_entity_ids is not None:
            if len(newly_inserted_entity_ids) == 0:
                logger.info("No newly inserted entities to compare, skipping synonymy edge creation.")
                return
            
            # Filter to only newly inserted entities that actually exist in the store
            query_entity_node_keys = [eid for eid in newly_inserted_entity_ids if eid in self.entity_id_to_row]
            
            if len(query_entity_node_keys) == 0:
                logger.info("No valid newly inserted entities found in store, skipping synonymy edge creation.")
                return
                
            logger.info(f"Comparing {len(query_entity_node_keys)} newly inserted phrase nodes against {len(all_entity_node_keys)} total phrase nodes.")
        else:
            # Fallback: compare all entities (quadratic, but maintains backwards compatibility)
            query_entity_node_keys = all_entity_node_keys
            logger.warning(f"No newly_inserted_entity_ids provided. Comparing all {len(query_entity_node_keys)} entities to all entities (quadratic scaling).")

        logger.info(f"Performing KNN retrieval for {len(query_entity_node_keys)} query phrase nodes against {len(all_entity_node_keys)} key phrase nodes.")

        query_entity_embs = self.entity_embedding_store.get_embeddings(query_entity_node_keys)
        all_entity_embs = self.entity_embedding_store.get_embeddings(all_entity_node_keys)

        # Build synonymy edges only between newly inserted phrase nodes and all phrase nodes in the storage to reduce cost for incremental graph updates
        query_node_key2knn_node_keys = retrieve_knn(query_ids=query_entity_node_keys,
                                                    key_ids=all_entity_node_keys,
                                                    query_vecs=query_entity_embs,
                                                    key_vecs=all_entity_embs,
    ...
    def get_top_k_weights(self,
        ...
        """
        # Handle NaN values in phrase weights (can occur from division by zero in caller)
        all_phrase_weights = np.nan_to_num(all_phrase_weights, nan=0.0)
        ...
        # Track which phrase IDs we're keeping for debugging
        kept_phrase_ids = set()
        for phrase_key in self.node_name_to_vertex_idx:
            phrase_id = self.node_name_to_vertex_idx.get(phrase_key, None)
            if phrase_id is not None:
                if phrase_key in top_k_phrases_keys:
                    kept_phrase_ids.add(phrase_id)
                else:
    ...
    def graph_search_with_fact_entities(self, query: str,
        ...
        # Only process phrases that actually exist in the graph (phrase_id is not None)
        for phrase, phrase_id in phrases_and_ids:
            # Skip phrases that don't exist in the graph
