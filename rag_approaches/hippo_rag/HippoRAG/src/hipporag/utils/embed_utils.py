...
def retrieve_knn(query_ids: List[str], key_ids: List[str], query_vecs, key_vecs, k=2047, query_batch_size=1000,
    ...
    """
    print(f"[DEBUG retrieve_knn] query_count={len(query_ids)}, key_count={len(key_ids)}, k={k}, query_batch={query_batch_size}, key_batch={key_batch_size}, total_comparisons={len(query_ids) * len(key_ids)}")
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    if torch.cuda.is_available():
        print(f"[DEBUG CUDA Memory] Total: {torch.cuda.get_device_properties(0).total_memory / 1e9:.2f} GB, Allocated: {torch.cuda.memory_allocated() / 1e9:.2f} GB, Reserved: {torch.cuda.memory_reserved() / 1e9:.2f} GB, Free: {(torch.cuda.get_device_properties(0).total_memory - torch.cuda.memory_reserved()) / 1e9:.2f} GB")
    else:
        print(f"[DEBUG CUDA Memory] CUDA not available, using CPU")
